...
API_URL = 'https://data.cms.gov/data-api/v1/dataset/44060663-47d8-4ced-a115-b53b4c270acb/data'
...
if __name__ == "__main__":
    raw_data = fetch_hospital_cost_data()
    if raw_data:
        preprocessed_data = preprocess_data(raw_data)
        if preprocessed_data:
            # Use environment variable for S3 bucket name
            BUCKET_NAME = os.getenv('S3_BUCKET_NAME', 'healthaibucket')
            store_in_s3(preprocessed_data, 'preprocessed_hospital_cost_data.json', bucket_name=BUCKET_NAME)
