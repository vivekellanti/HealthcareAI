...
import logging
import os
from src.utils.cms_data_utils import preprocess_data, store_in_s3
...
if __name__ == "__main__":
    raw_data = fetch_outpatient_data()
    if raw_data:
        preprocessed_data = preprocess_data(raw_data)
        if preprocessed_data:
            # Use environment variable for S3 bucket name
            BUCKET_NAME = os.getenv('S3_BUCKET_NAME', 'healthaibucket')
            store_in_s3(preprocessed_data, 'preprocessed_outpatient_data.json', bucket_name=BUCKET_NAME)
